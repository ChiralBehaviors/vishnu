...
/**
 * Adapt {@link InetSocketAddress} so that it can be used in a similar way to
 * {@link Service} when obtaining host and port. If we didn't register this
 * adaptor then in order get the host portion of the {@link InetSocketAddress}
 * we would have to access it via the "hostname" property/getter which would be
 * inconsistent with how we get the host portion of the configuredService
 * variable
 * 
 */
